...
if ($result && $result->success) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (preg_match($urlPattern, $patient)) {
            $errors[] = "Patient must not contain URLs.";
        }
        ...
        if (!empty($errors)) {
            ...
            echo "<script>alert('" . $errorText . "'); window.location='" . SITE_URL . "';</script>";
            exit;
        }        

        if (!empty($email) && !empty($fname)) {

            $to      = 'info@wecare.com,wecare@gmail.com';
...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background-color:#f8f9fa;">
    <table width="100%" border="0" cellspacing="0" cellpadding="0"
        ...
        <tr>
            ...
            echo '<script>window.location="' . SITE_URL . 'thankyou";</script>';
            exit;
